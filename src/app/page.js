...
function PageContent({ activeNav, setActiveNav, conf }) {
  switch (activeNav) {
    case 'committee': return <OrganizingCommittee conf={conf} />;
    ...
    case 'work-reports': return <WorkReports conf={conf} />;
    case 'positives': return <Positives conf={conf} />;
    case 'website-sections': return <WebsiteSections conf={conf} />;
    ...
    case 'ws-marquee': return <WebsiteSections section="marquee" conf={conf} />;
    case 'ws-pricing': return <WebsiteSections section="pricing" conf={conf} />;
    case 'ws-partners': return <WebsiteSections section="partners" conf={conf} />;
...
export default function Home() {
  ...
  useEffect(() => {
    // 1. Session Restoration with Expiry Check (30 Minute TTL)
    ...
    if (saved) {
      try {
        ...
        // If session is older than 30 mins, clear it
        if (!info.timestamp || (now - info.timestamp) > thirtyMinutes) {
